classdef Ruby < handle
    ...
    methods
        %% Constructor
        function self = Ruby(gemCount, positions)
            if nargin > 0
            ...
            for i = 1:self.gemCount
                ...
                self.gemPatch{i} = self.CreateAndPlotGem('RedRuby.ply', basePose);
    ...
    methods (Static)
        ...
        function gemPatch = CreateAndPlotGem(plyFileName, basePose)
            ...
            if isfield(plyData.vertex, 'red')
                ...
                vertexColors = repmat([1, 0, 0], size(vertexData, 1), 1);  % Default color (red)
            end
