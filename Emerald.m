classdef Emerald < handle
    ...
    methods
        %% Constructor
        function self = Emerald(gemCount, positions)
            if nargin > 0
            ...
            for i = 1:self.gemCount
                ...
                self.gemPatch{i} = self.CreateAndPlotGem('GreenEmerald.ply', basePose);
    ...
    methods (Static)
        ...
        function gemPatch = CreateAndPlotGem(plyFileName, basePose)
            ...
            if isfield(plyData.vertex, 'red')
                ...
                vertexColors = repmat([0, 1, 0], size(vertexData, 1), 1);  % Default color (green)
            end
